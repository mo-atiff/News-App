...
def stream():
    ...
    with col3:
        src = st.selectbox('FROM(SOURCE)', ['Chennai', 'Delhi', 'Kolkata', 'Mumbai', 'Bangalore'])
        st.write(f'SOURCE : {src}')

    with col4:
        dest = st.selectbox('TO(DESTINATION)', ['Cochin', 'Delhi', 'Hyderabad', 'Kolkata', 'New Delhi', 'Bangalore'])
